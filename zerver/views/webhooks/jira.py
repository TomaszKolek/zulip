...
def api_jira_webhook(request, user_profile):
    ...
    if baseUrl and len(baseUrl.groups()):
        issue = "[{}]({}/browse/{})".format(issueId, baseUrl.group(1), issueId)
    else:
        issue = issueId
    ...
    if event == 'jira:issue_created':
        ...
        # event that happened
        assignee_blurb = '' if assignee_mention == '' else " (assigned to {})".format(assignee_mention)
        ...
        content = "{} **transitioned** {} from {} to {}".format(author, issue, from_status, to_status)
    else:
        # Unknown event type
