...
def fill_format_dict_by_message(format_dict, message, key):
    user = "{}_user".format(key)
    username = "{}_username".format(key)
    email = "{}_email".format(key)
    url = "{}_url".format(key)

    if message['data']['incident'].get(user):
        format_dict[email] = message['data']['incident'][user]['email']
        format_dict[username] = message['data']['incident'][user]['email'].split('@')[0]
        format_dict[url] = message['data']['incident'][user]['html_url']
    else:
        format_dict[email] = 'nobody'
        format_dict[username] = 'nobody'
        format_dict[url] = ''
...
def build_pagerduty_formatdict(message):
    ...
    format_dict = fill_format_dict_by_message(format_dict, message, 'assigned_to')
    format_dict = fill_format_dict_by_message(format_dict, message, 'resolved_by')
...
def send_raw_pagerduty_json(user_profile, stream, message, topic):
    ...
    body = (
        ...
        u'``` py\n'
        u'{}\n'
        u'```').format(pprint.pformat(message),)
    check_send_message(user_profile,
                       get_client('ZulipPagerDutyWebhook'),
                       'stream',
                       [stream],
                       subject,
                       body)
