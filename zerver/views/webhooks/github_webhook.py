...
def get_push_body(payload):
    ...
    return get_push_commits_event_message(
        get_sender_name(payload),
        payload['compare'],
        get_branch_name_from_href(payload['ref']),
        commits_data
    )
...
def get_branch_name_from_href(href_string):
    # type: (text_type) -> text_type
    return re.sub(r'^refs/heads/', '', href_string)
...
def get_subject_based_on_type(payload, event):
    ...
    if event == 'push':
        return SUBJECT_WITH_BRANCH_TEMPLATE.format(
            repo=get_repository_name(payload),
            branch=get_branch_name_from_href(payload['ref'])
        )
    if event == 'gollum':
        return SUBJECT_WITH_BRANCH_TEMPLATE.format(
            repo=get_repository_name(payload),
...
def api_github_webhook(
        ...
        payload=REQ(argument_type='body'), stream=REQ(default='github-webhook')):
