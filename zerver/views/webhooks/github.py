...
def api_github_v2(user_profile,
    ...
    if event == 'pull_request':
        ...
        comment = payload['comment']
        subject = "{}: commit {}".format(topic_focus, comment['commit_id'])

        content = "{} [commented]({})".format(comment['user']['login'], comment['html_url'])

        if comment['line'] is not None:
            content += " on `{}`, line {}".format(comment['path'], comment['line'])
...
def api_github_landing(request, user_profile, event=REQ,
    ...
    stream = 'commits' if not stream else stream
    ...
    if version == 2:
        target_stream, subject, content = api_github_v2(user_profile,
                                                        event,
                                                        payload,
                                                        branches,
                                                        stream,
        ...
        target_stream, subject, content = api_github_v1(user_profile,
                                                        event,
                                                        payload,
                                                        branches,
                                                        stream,
...
def build_commit_list_content(commits, branch, compare_url, pusher):
    ...
    if num_commits > COMMITS_IN_LIST_LIMIT:
        content += "\n[and {} more commits]".format(num_commits - COMMITS_IN_LIST_LIMIT)
    return content
