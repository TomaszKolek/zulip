    ...
    def _wrapped_view_func(request, *args, **kwargs):
        try:
            ...
            if auth_type.lower() == "basic":
                ...
                request.META['HTTP_AUTHORIZATION'] = "Basic {}".format(base64.b64encode("{}:{}".format(email, api_key)))
        except:
            pass
...
def api_beanstalk_webhook(request, user_profile,
    ...
    if is_git_repo:
        ...
        author = payload['author_full_name']
        url = payload['changeset_url']
        revision = payload['revision']
        short_commit_msg = payload.get('message').partition("\n")[0]

        subject = "svn r{}".format(revision)
        content = "{} pushed [revision {}]({}):\n\n> {}".format(author, revision, url, short_commit_msg)

    check_send_message(user_profile, get_client("ZulipBeanstalkWebhook"), "stream", ["commits"], subject, content)
    return json_success()
